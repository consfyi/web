    ...
    const months = useMemo(() => {
        ...
        for (
            let d = setDate(reinterpretAsLocalDate(cons![0].start), 1),
                endDate = addMonths(
                    setDate(
                        reinterpretAsLocalDate(cons![cons!.length - 1].start),
                        1
                    ),
                    1
                );
            ...
            if (
                hideEmptyGroups &&
                (consByMonth[yearMonthKey(d)] ?? []).length == 0
            ) {
                continue;
            }
            months.push(d);
                            ...
                            >
                                <Text span size="xs" fw={500}>
                                    {continentsFiltered ? (
                                        ...
                                        1 ? (
                                            continentStrings[
                                                viewOptions.filter.continents[0]
                                            ]
                                        ) : (
                                            <Plural
                                                value={
                    ...
                    {viewOptions.layout != "calendar" ? (
                        ...
                        >
                            ...
                            <Menu.Dropdown w={200}>
                                ...
                                {Object.values(SortBy.def.entries).map(
                                    (sortBy) => {
                                        if (
                                            !isLoggedIn &&
                                            sortBy == "followed"
                                            ...
                                            <Menu.Item
                                                ...
                                                onClick={() => {
                                                    setViewOptions((vo) => ({
                                                        ...
                                                        layout: {
                                                            sort: sortBy,
                                                            desc: DEFAULT_SORT_DESC_OPTIONS[
                                                                sortBy
                                                            ],
                                                        },
                                                    }));
                ...
                {sortedContinents.map((code) => {
                    return (
                        <Checkbox
                            key={code}
                            mb="sm"
                            checked={viewOptions.filter.continents.includes(
                                code
                            )}
                            onChange={(e) => {
                                setViewOptions({
...
export default function ConsList({ cons }: { cons: ConWithPost[] }) {
    ...
    const actuallyShowOnlyAttending =
        isLoggedIn && viewOptions.filter.attending;
